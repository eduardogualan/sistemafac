...
{block name="contenido"}
    <div class="portlet light bordered" id="form_wizard_1">
        ...
        <div class="portlet-body form">
            <form   onsubmit="return validar()"class="form-horizontal form-label-left" action="{base_url()}usuarios/guardar" method="POST" autocomplete="off">
                    ...
                    <div class="col-md-3 col-sm-3 col-xs-6">
                        <select class="chosen-select form-control" id="rol" name="rol" onchange="validacion('rol')">
                            ...
                            {foreach from=$lista_rol item=datos}
                                {if !(($datos->nombre== "Administrador") || ($datos->nombre== "Cliente"))}
                                <option value="{$datos->id_rol}">{$datos->nombre}</option>
                                {/if}
                            {/foreach}
