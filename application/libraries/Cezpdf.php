...
//***********
function ezHeader($x,$y,$size,$pos='left',$pattern,$num='',$linea,$img){
  ...
  $this->ez['pageNumbering'][$i][$this->ezPageCount]=array('x'=>$x,'y'=>$y,'pos'=>$pos,'pattern'=>$pattern,'num'=>$num,'size'=>$size,'linea'=>$linea,'img'=>$img);
  return $i;
}
//**************---------
/*
        ...
        if ($status){
          ...
          switch($info['pos']){
            case 'right':
              ...
              $this->addText($x,$y,$size,$titulo);
              break;
            default:
              $w=$this->getTextWidth($size,$titulo);
              $this->addText($x-$w,$y,$size,$titulo);
              break;
          }
          $this->closeObject();
        }
...
function ezStartPageNumbers($x,$y,$size,$pos='left',$pattern='{PAGENUM} of {TOTALPAGENUM}',$num=''){
  ...
  $this->ez['pageNumbering'][$i][$this->ezPageCount]=array('x'=>$x,'y'=>$y,'pos'=>$pos,'pattern'=>$pattern,'num'=>$num,'size'=>$size,'linea'=>false,'img'=>'');
  return $i;
}
        ...
        if ($status){
          ...
          switch($info['pos']){
            case 'right':
              $this->addText($info['x'],$info['y'],$info['size'],$pat);
              if($info['linea']==true){
              	$this->line($this->ez['leftMargin'], $info['y']-5, $this->ez['pageWidth']-$this->ez['rightMargin'], $info['y']-5);//the bottom line
              }
              break;
            default:
